...
def fix_untrained_tokens(model, tokenizer, train_dataset, eps = 1e-16):
    ...
    input_emb   = model.get_input_embeddings () # Input Embeddings
    output_emb  = model.get_output_embeddings() # Output Embeddings
    
    if output_emb is not None:
        embedding_matrix = input_emb.weight
        lm_head_matrix = output_emb.weight
    else:
        embedding_matrix = input_emb.weight
        lm_head_matrix = None
    ...
    if lm_head_matrix is not None:
        lm_head_matrix.requires_grad: bad_not_trainable = True
    else: bad_not_trainable = False
    ...
    # Embeddding Matrix 
    sum_embedding = torch.sum(embedding_matrix, dtype = torch.float32, axis = 0)                        # Get sum of all items
    sum_embedding -= torch.sum(embedding_matrix[where_untrained], dtype = torch.float32, axis = 0)      # Remove bad tokens
    mean_embedding = (sum_embedding / n_trained)                                                        # Find correct average by dividing by sum of trained tokens
    
    if lm_head_matrix is not None:
        # LM Head
        sum_lm_head   = torch.sum(lm_head_matrix,   dtype = torch.float32, axis = 0)                    # Get sum of all items
        sum_lm_head   -= torch.sum(lm_head_matrix  [where_untrained], dtype = torch.float32, axis = 0)  # Remove bad tokens
        mean_lm_head   = (sum_lm_head   / n_trained)                                                    # Find correct average by dividing by sum of trained tokens
    ...
    if lm_head_matrix is not None:      lm_head_matrix  [where_untrained] = mean_lm_head  .to(lm_head_matrix  .dtype)
