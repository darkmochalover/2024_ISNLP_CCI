...
class CustomDataset(Dataset):
    def __init__(self, fname, model_type, tokenizer, category=args.category, seed=42):
        ...
        for example in data:
            if category is None or example['input']['category'] == category:
                ...
                self.inp.append(source['input_ids'].squeeze(0))
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances):
        input_ids = torch.nn.utils.rnn.pad_sequence(
            [torch.tensor(instance['input_ids']) for instance in instances], batch_first=True, padding_value=self.pad_token_id
        )
